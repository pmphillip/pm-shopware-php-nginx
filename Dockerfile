...
RUN apt-get autoclean && apt-get update -qqy && apt -y --fix-broken install python-pycurl python-apt && apt-get install -qqy software-properties-common
...
RUN apt-get update && apt-get install -y libmagickwand-6.q16-dev --no-install-recommends \
    ...
    && pecl install apcu \
    && echo "extension=apcu.so" > /etc/php/7.4/fpm/conf.d/20-apcu.ini \
    && rm -rf /var/lib/apt/lists/*
...
COPY ./php-conf/custom-php.ini /etc/php/7.4/cli/conf.d/10-shopware.ini
COPY ./php-conf/custom-php.ini /etc/php/7.4/fpm/conf.d/10-shopware.ini
