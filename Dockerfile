...
RUN apt-get update && apt-get install -y libmagickwand-6.q16-dev --no-install-recommends \
    ...
    && echo "extension=imagick.so" >  /etc/php/7.3/fpm/conf.d/20-imagick.ini \
    && pecl install apcu \
    && echo "extension=apcu.so" > /etc/php/7.3/fpm/conf.d/20-apcu.ini \
    && rm -rf /var/lib/apt/lists/*
...
RUN sed -i -e "s/;cgi.fix_pathinfo=1/cgi.fix_pathinfo=0/g" /etc/php/7.3/fpm/php.ini && \
    ...
    sed -i -e "s/cgi.fix_pathinfo=0/cgi.fix_pathinfo=1/g" /etc/php/7.3/fpm/php.ini && \
    sed -i -e "s/upload_max_filesize\s*=\s*2M/upload_max_filesize = 100M/g" /etc/php/7.3/cli/php.ini && \
    sed -i -e "s/post_max_size\s*=\s*8M/post_max_size = 100M/g" /etc/php/7.3/cli/php.ini && \
...
RUN tar xvzfC /tmp/php7-linux-x86-64-beta8.tgz /tmp/ \
    ...
    && cp /tmp/ioncube/ioncube_loader_lin_7.3.so /usr/local/ioncube \
