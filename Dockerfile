...
RUN apt-get autoclean && apt-get update -qqy && apt-get install -qqy software-properties-common
...
RUN sed -i -e "s/;cgi.fix_pathinfo=1/cgi.fix_pathinfo=0/g" /etc/php/7.4/fpm/php.ini && \
    ...
    sed -i -e "s/cgi.fix_pathinfo=0/cgi.fix_pathinfo=1/g" /etc/php/7.4/fpm/php.ini && \
...
#RUN chown -R www-data:www-data /var/log/php7.4-fpm.log
